...
type queueRecallSkippedForm struct {
	CounterID int   `form:"counter_id" json:"counter_id" binding:"required"`
	TicketID  int64 `form:"ticket_id" json:"ticket_id" binding:"required"`
}
...
func QueueRecallSkipped(c *gin.Context) {
	var form queueRecallSkippedForm
	if err := c.ShouldBind(&form); err != nil || form.CounterID <= 0 || form.TicketID <= 0 {
		c.JSON(http.StatusBadRequest, gin.H{"ok": false, "message": "counter_id atau ticket_id tidak valid"})
		return
	}
	...
	_, ticketNo, err := service.RecallSkipped(counter.StoreID, form.CounterID, userID, form.TicketID)
	if err != nil {
		switch err {
		case services.ErrNoSkippedTicket:
			c.JSON(http.StatusBadRequest, gin.H{"ok": false, "message": "Tidak ada antrian terlewat"})
			return
		default:
			c.JSON(http.StatusInternalServerError, gin.H{"ok": false, "message": err.Error()})
			return
	...
	payload, payloadErr := buildQueueViewPayload(counter.StoreID, "recall_skipped")
	if payloadErr == nil {
	...
	resp := gin.H{
		"ok":        true,
		"message":   "Nomor terlewat dipanggil kembali",
		"ticket_no": ticketNo,
	}
	if payloadErr == nil {
		resp["data"] = payload
